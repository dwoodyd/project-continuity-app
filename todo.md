...
## A2 Auth Blockers (Jun 2026 — confirmed via incognito test)

- [ ] A2-BLOCKER-1 (owner action): Register `https://app.continuary.app/api/oauth/callback` as an authorized redirect URI in the Manus OAuth app settings (Manus dashboard → your OAuth app → Authorized redirect URIs). Until this is done, every new user in a clean session gets "Permission denied — Redirect URI is not set." This is a platform config action, not a code change.
- [ ] A2-BLOCKER-2 (decision required): The Manus-hosted login page at manus.im shows "Powered by Manus," "from Meta," and "©2026 Meta." Options: (a) check if your Manus plan supports white-labeling the OAuth page, or (b) do the full auth provider swap (Clerk/Auth0/Google direct) — see Section A2 of the build list. Code is ready for either path; only the upstream identity provider changes.
- [ ] A2-RETEST: After Blocker 1 is fixed, re-test in a fresh incognito window: sign-in should reach the login card without "Permission denied." Then decide Blocker 2 path.

## Clerk Auth Swap — Completed (Jun 2, 2026)

- [x] CLERK-1: Install @clerk/express and @clerk/clerk-react SDKs
- [x] CLERK-2: Backend — mount clerkMiddleware() on Express app before all routes
- [x] CLERK-3: Backend — rewrite sdk.authenticateRequest to use getAuth(req) + clerkClient.users.getUser + upsertUser
- [x] CLERK-4: Backend — removed jose session minting; auth.logout simplified to cookie clear only
- [x] CLERK-5: Frontend — wrap app in ClerkProvider with VITE_CLERK_PUBLISHABLE_KEY
- [x] CLERK-6: Frontend — gate app with <SignedIn>/<SignedOut><RedirectToSignIn /></SignedOut>
- [x] CLERK-7: Frontend — getLoginUrl() returns "/sign-in"; useAuth.logout calls Clerk signOut()
- [x] CLERK-8: /api/oauth/callback stubbed to no-op; ManusDialog unused; OAUTH_SERVER_URL not used in auth path
- [x] CLERK-9: TypeScript: 0 errors | Tests: 383/383 passing | Checkpoint saved
- [ ] OWNER ACTION: In Clerk Dashboard → Configure → Paths → Add app.continuary.app to Allowed redirect URLs
- [ ] OWNER ACTION: In Clerk Dashboard → Configure → SSO Connections — verify Google and Email/Password are enabled
- [ ] OWNER ACTION (optional): Set custom domain in Clerk Dashboard to remove .clerk.accounts.dev from sign-in URL
